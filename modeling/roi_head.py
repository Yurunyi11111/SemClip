...
class ClipRes5ROIHeads(Res5ROIHeads):   
    ...
    def forward(
        ...
        scores_re, proposal_deltas_re = predictions
        # scores_re, proposal_deltas_re = self.box_predictor(box_features.mean(dim=[2, 3]))
        # predictions = (scores_re, proposal_deltas_re)
        # scores_re = self.box_predictor(box_features.mean(dim=(2,3)))
        ...
        if self.training:
            ...
            return [], losses,scores_re
        else:
            pred_instances, _ = self.box_predictor.inference(predictions, proposals)
            pred_instances = self.forward_with_given_boxes(features, pred_instances)
            return pred_instances, {},scores_re
...
class ClipRes5ROIHeadsAttn(ClipRes5ROIHeads): 
    ...
    def forward(
        ...
        # scores_re, proposal_deltas_re = self.box_predictor([attn_feat,box_features.mean(dim=(2,3))])
        # predictions = (scores_re, proposal_deltas_re)
        # scores_re = self.box_predictor(box_features.mean(dim=(2,3)))
        # import pdb;pdb.set_trace()
        if self.training:
